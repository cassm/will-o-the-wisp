...
for i in range(paletteLength):
    ...
    for j in range(3):
        ...
        for k in range(int(-kernelSize), int(kernelSize) + 1):
            ...
            if x <= 2:
                weighting = 1 / 6.0 * (
                    (12 - 9 * b - 6 * c) * (abs(x) ** 3) + (-18 + 12 * b + 6 * c) * (x ** 2) + (6 - 2 * b))
            elif x <= 1:
                weighting = 1 / 6.0 * (
                    (-b - 6 * c) * (abs(x) ** 3) + (6 * b + 30 * c) * (x ** 2) + (-12 * b - -48 * c) * x + (
                        8 * b + 24 * c))
...
f.write(
    "#ifndef {0}\n#define {0}\n\n// RGBW file generated via genPalette.py, created by Cass May\n\n#include <stdint.h>\n\n".format(
        paletteName.upper() + "_PALETTE_H"))

f.write("int {}PaletteLen = {};\n".format(paletteName, paletteLength))
f.write("uint16_t {}[{}][4] = {};\n\n".format(paletteName + "RgbwPalette", paletteLength,
                                              str(output).replace('[', '{').replace(']', '}')))
f.write("double {}[{}][3] = {};\n\n".format(paletteName + "HsiPalette", paletteLength,
                                            str(hsiOutput).replace('[', '{').replace(']', '}')))
